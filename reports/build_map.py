...
svg{display:block;width:100%;height:auto;aspect-ratio:960/640;touch-action:manipulation;background:var(--surface-2)}
...
.dot.pos{fill:var(--case)} .dot.neg{fill:var(--control)}
...
.glossary{margin:14px 0 0;background:var(--surface);border:1px solid var(--line);border-radius:12px;padding:0 16px;box-shadow:var(--shadow)}
.glossary summary{cursor:pointer;font-weight:600;font-size:.9rem;padding:13px 0;color:var(--ink);list-style-position:inside}
.glossary summary::-webkit-details-marker{color:var(--case)}
.glossary dl{margin:0 0 14px;font-size:.85rem}
.glossary dt{font-weight:650;color:var(--ink);margin-top:12px}
.glossary dd{margin:2px 0 0;color:var(--ink-2);line-height:1.5}
...
<div class="wrap"><div class="inner">
  ...
  <details class="glossary">
    <summary>What the numbers in each popup mean</summary>
    <dl>
      <dt>PCR-positive — ordered to clean</dt>
      <dd>The building's cooling tower screened positive for <em>Legionella</em> on a rapid PCR
      test, and the Health Department ordered it cleaned &amp; disinfected. (PCR can't tell live
      from dead bacteria, so cleaning is precautionary.)</dd>
      <dt>Registered · not on DOH list</dt>
      <dd>A cooling tower registered with the city that was <em>not</em> on that clean/disinfect
      list — the comparison group we measure the positives against.</dd>
      <dt>ZIP</dt>
      <dd>The building's postal code (all within the three outbreak ZIPs: 10028 / 10075 / 10128).</dd>
      <dt>Cooling towers</dt>
      <dd>How many separate cooling-tower systems the building has registered with NYC.</dd>
      <dt>Registered (yrs ago)</dt>
      <dd>How long ago the building first registered a cooling tower — a rough age of its cooling
      system on record. Citywide registration began in 2015, so this maxes out around then.</dd>
      <dt>Days since last test</dt>
      <dd>Days between the building's most recent routine <em>Legionella</em> water sample and the
      start of the July 2026 investigation. Owners are required to sample on a set schedule.</dd>
      <dt>Violations (all-time)</dt>
      <dd>Total cooling-tower regulatory violations the building has received in city inspections
      since 2017 — paperwork and water-management issues alike.</dd>
    </dl>
  </details>
...
function draw(){
  ...
  svg.appendChild(lab);
  // --- dots (visible marks, pointer-events off) ---
  const shown=P.filter(p=>(p.c?state.case:state.ctrl)).sort((a,b)=>a.c-b.c);
  const radius=p=>state.sizeByTowers?4.5+Math.sqrt(p.nt)*2.3:5.5;
  shown.forEach(p=>{
    const c=document.createElementNS(NS,'circle');
    c.setAttribute('cx',SX(p));c.setAttribute('cy',SY(p));c.setAttribute('r',radius(p));
    c.setAttribute('fill-opacity',p.c?'.92':'.78');
    c.setAttribute('class','dot '+(p.c?'pos':'neg'));c.setAttribute('pointer-events','none');
    svg.appendChild(c);
  });
  // --- large transparent hit targets on top, for reliable tap/click ---
  shown.forEach(p=>{
    const h=document.createElementNS(NS,'circle');
    h.setAttribute('cx',SX(p));h.setAttribute('cy',SY(p));
    h.setAttribute('r',Math.max(radius(p)+7,12));
...
function place(e){
  ...
  const th=tip.offsetHeight||150, tw=tip.offsetWidth||210;
  const above = y-th-14 >= 0;                 // only go above if it fits inside the map
...
function hideTip(){tip.classList.remove('on');pinned=false;}
function stats(){
...
svg.addEventListener('click',hideTip);   // tap empty map to dismiss
